...
async function getBlock(block: primitives.Block) {
  ...
  try {
    ...
    nearDBIndexUpdates = contractActions.flatMap((action) =>
      action.operations
      ...
      nearDBIndexUpdates.map(async (writeAction) => {
        ...
        if (Object.keys(writeAction.args.data[accountId]).includes("index")) {
          if (!writeAction.args.data[accountId].index) {
            console.log(
              `Wrong notify format for receiver: ${accountId}, index is null`
            );
            return;
          }
          if (
...
on_conflict: {
      ...
      console.log(
        `Failed to store Notify record for ${accountId}. Error: ${e}`
      );
    }
  }
