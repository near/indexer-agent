...
|\r\
  ...
  const componentUpdateTransactions = nearSocialActions.filter(
    (functionCall) => {
      ...
      return Object.keys(accountData).includes("widget");
    }
  );

  const componentStarTransactions = nearSocialActions.filter((functionCall) => {
    if (
      !functionCall ||
      !functionCall.args ||
      !functionCall.args.data ||
    ...
    for (const transaction of componentUpdateTransactions) {
      ...
      for (const componentName of componentNames) {
        ...
        if (forkSource) {
          ...
          const previousInfoRawResponse =
            rawGraphqlResponseForkSource[METADATA_TABLE] ?? [];

          if (
            previousInfoRawResponse &&
            previousInfoRawResponse.length &&
            previousInfoRawResponse[0]
          ) {
            const {
              block_height,
              block_timestamp_ms,
            ...
            if (
              ...
              await context.db.Metadata.upsert(
                [source_info],
                ["component_id"],
                ["fork_count"]
              );
  ...
  if (componentStarTransactions.length > 0) {
    ...
    for (const transaction of componentStarTransactions) {
      const componentAuthorId = Object.keys(transaction.args.data)[0];
      const rawComponentData =
        transaction.args.data[componentAuthorId]["index"] || {};
      const rawStarData = rawComponentData?.star || {};
      if (!rawStarData || !Object.keys(rawStarData).length) return;

      const star_type =
        typeof rawStarData === "string" ? JSON.parse(rawStarData) : rawStarData;
      if (!star_type) return;
      ...
      if (
        component_id &&
        (transaction_value === STAR_TRANSACTION_TYPE ||
          transaction_value === UNSTAR_TRANSACTION_TYPE)
      ) {
        ...
        const previousInfoRawResponse =
          rawGraphqlResponseStarData[METADATA_TABLE] ?? [];
        ...
        console.log(
          "one or more pieces of essential metadata missing to increment/decrement star count: star_type is",
          star_type,
          " component_id: ",
          component_id
        );
