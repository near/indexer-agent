...
def data_upsertion_code_model():
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            ("human",
             """
             To improve coding practices, define all helper functions at the beginning of the script before the main logic. Declare constants, such as SOCIAL_DB, inside the main function but outside the primary logic block for clarity. Use a detailed and nested approach to parse and handle data, including filtering operations and handling specific cases as needed. Explicitly handle different actions (posts, comments, likes, promotions, and reposts) with separate, well-defined functions. Include detailed logging and error messages for various failure cases to aid in debugging and provide clearer information. Ensure functions for handling reposts and promotions are included and properly implemented. Include helper functions for handling likes (_handlePostLike, _handlePostUnlike) and place them before the main logic. Check and decode arguments in a detailed manner to ensure data is in the expected format before processing. For example, when handling post creation, first check if the post already exists in the database. If it does not, proceed to insert the post and log success; otherwise, log that the post already exists. Use this structured approach to create robust, maintainable, and clear code, ensuring all edge cases and potential errors are handled appropriately.""",
            ),
            ("ai",
             """
            import * as primitives from "@near-lake/primitives";
