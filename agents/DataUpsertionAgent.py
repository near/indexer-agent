...
import json
import os
from prompts import (data_upsertion_system_prompt,
...
DataUpsertion_parser = PydanticOutputParser(
    pydantic_object=DataUpsertionResponse)
...
def data_upsertion_code_model():
    ...
    model = {"messages": RunnablePassthrough()} | prompt | llm.with_structured_output(
        DataUpsertionResponse)  # .bind_tools(tools)
    return model
